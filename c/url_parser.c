 ...
 * When we see one of these as the last two labels, the registrable
 * domain includes one more label.
 */
static const char *TWO_PART_TLDS[] = {
    "co.uk", "co.jp", "co.kr", "co.nz", "co.za", "co.in", "co.id",
    "com.au", "com.br", "com.cn", "com.mx", "com.sg", "com.tw",
    "com.ar", "com.hk", "com.my", "com.ph", "com.pk", "com.co",
...
static int is_two_part_tld(const char *tld_candidate) {
    for (int i = 0; TWO_PART_TLDS[i]; i++) {
        if (strcasecmp(tld_candidate, TWO_PART_TLDS[i]) == 0) return 1;
    }
    return 0;
}
...
int url_extract_domain(const ParsedUrl *url, DomainInfo *info) {
    ...
    /* Skip IP addresses */
    if (url->is_ipv4 || url->is_ipv6) {
        strncpy(info->registrable_domain, info->full_host,
                sizeof(info->registrable_domain) - 1);
        return 0;
    }

    /* Split host by dots */
    char buf[URL_MAX_HOST];
    strncpy(buf, info->full_host, sizeof(buf) - 1);
    buf[sizeof(buf) - 1] = '\0';

    char *labels[32];
    int label_count = 0;
    char *p = buf;
    while (*p && label_count < 32) {
        labels[label_count++] = p;
        char *dot = strchr(p, '.');
        if (dot) {
            *dot = '\0';
            p = dot + 1;
    ...
    /* Single label (e.g. "localhost") */
    if (label_count == 1) {
        strncpy(info->tld, labels[0], sizeof(info->tld) - 1);
        strncpy(info->registrable_domain, info->full_host,
                sizeof(info->registrable_domain) - 1);
        return 0;
    }

    /* TLD is the last label */
    strncpy(info->tld, labels[label_count - 1], sizeof(info->tld) - 1);

    /* Check for two-part TLD */
    int reg_start = label_count - 2;
    if (label_count >= 3) {
        char two_part[128];
        snprintf(two_part, sizeof(two_part), "%s.%s",
                 labels[label_count - 2], labels[label_count - 1]);
        if (is_two_part_tld(two_part)) {
            reg_start = label_count - 3;
            strncpy(info->tld, two_part, sizeof(info->tld) - 1);
        }
    }

    if (reg_start < 0) reg_start = 0;

    /* Build registrable domain from reg_start onward */
    /* We need to reconstruct from the original host since we split buf */
    const char *host_ptr = info->full_host;
    for (int i = 0; i < reg_start; i++) {
        const char *dot = strchr(host_ptr, '.');
        if (dot) host_ptr = dot + 1;
    }
    strncpy(info->registrable_domain, host_ptr,
            sizeof(info->registrable_domain) - 1);
    info->registrable_domain[sizeof(info->registrable_domain) - 1] = '\0';

    /* Extract subdomain (everything before the registrable domain) */
    size_t sub_len = (size_t)(host_ptr - info->full_host);
    if (sub_len > 0) {
        if (sub_len > sizeof(info->subdomain) - 1)
            sub_len = sizeof(info->subdomain) - 1;
        memcpy(info->subdomain, info->full_host, sub_len);
        /* Remove trailing dot */
        if (sub_len > 0 && info->subdomain[sub_len - 1] == '.')
            sub_len--;
        info->subdomain[sub_len] = '\0';
    }
